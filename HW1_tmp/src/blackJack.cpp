...
int blackJack::pOpt(int pIndex){
	...
	while(1){
		...
		else if(opt==3 && pbBet[pIndex]<=pBet[pIndex]){
...
void blackJack::showCards(){
	for(int i=0;i<PLAYER;i++){
		cout << pName[i] << "[Bet " << pbBet[i] << " coints / " << pBet[i] << " coins] Total Points: " << pPoint[i] << endl;
		cout << "\t" << getCards(i) << endl << endl;
	}
}
...
string blackJack::getName(int pIndex){
	return pName[pIndex];
}
